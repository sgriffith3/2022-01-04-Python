...
def get_names() -> list:
    r = requests.get("http://pokeapi.co/api/v2/pokemon/?limit=1119").json()
    # print(r)
    print(r['results'][0]['name'])
    names = []
    for poke in r['results']:
        names.append(poke['name'])
    return names


def names_out(names: list):
    # text file
    with open("pokenames.txt", "w") as f:
        for name in names:
            f.write(f"{name}\n")

    pokes = {"Name": names}
    # print(pokes)
    pokedf = pandas.DataFrame(pokes)
    print(pokedf)

    #json
    pokedf.to_json("pokenames.json")

    #excel
    pokedf.to_excel("pokenames.xlsx")
